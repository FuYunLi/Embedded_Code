...
static event_t event_enter = {EVENT_STATE_ENTER};
static event_t event_exit = {EVENT_STATE_EXIT};
...
void fsm_init(fsm_t *self, event_t *event)
{
    ...
    (*self->state)(self, &event_enter); /* 进入默认状态 */
}
...
void fsm_dispatch(fsm_t *self, event_t *event)
{
    ...
    {
        (*prev_state)(self, &event_exit);
        (*self->state)(self, &event_enter);
    }
}
